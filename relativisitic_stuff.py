...
class World:
    ...
    def update(self, dt):
        ...
        gamma = get_gamma(self.reference_frame_velocity, self.speedoflight)

        # Apply length contraction to scaling factor
        self.meters_per_pixel = (self.speedoflight * self.scaling_factor) / (self.screen_WIDTH * gamma)
    ...
    class Particle:
        ...
        def update(self, dt, meters_per_pixel, reference_frame_velocity):
            ...
            gamma = get_gamma(self.velocity, self.world.speedoflight)
            adjusted_velocity = (self.velocity - reference_frame_velocity) / (
                        1 - (self.velocity * reference_frame_velocity) / self.world.speedoflight ** 2)
            self.time = (self.world.time - (self.velocity*self.position)/(self.world.speedoflight**2)) * gamma
        ...
        def draw(self, screen):
            adjusted_velocity = (self.velocity - self.world.reference_frame_velocity) / (
                        1 - (self.velocity * self.world.reference_frame_velocity) / self.world.speedoflight ** 2)
    ...
    class Photon_emiter:
        ...
        def update(self, dt, meters_per_pixel, reference_frame_velocity, current_time):
            ...
            # Adjust velocity using relativistic velocity addition
            adjusted_velocity = (self.velocity - reference_frame_velocity) / (
                        1 - (self.velocity * reference_frame_velocity) / self.world.speedoflight ** 2)

            # Apply time dilation to the emission interval
            dilated_period = self.emission_interval * gamma

            # Update position using the relativistic velocity
            self.position = (self.position + (adjusted_velocity * dt) / meters_per_pixel) % self.world.screen_WIDTH

            # Emit photons based on the new time-dilated period
            if current_time - self.last_emission_time >= dilated_period:
...
sensor_arrangement_speed = 0.9*world.speedoflight
...
emitter = world.Photon_emiter(world, setup_position=500, velocity=sensor_arrangement_speed, period=1)
...
world.reference_frame_velocity = 0.9*world.speedoflight
world.run()
