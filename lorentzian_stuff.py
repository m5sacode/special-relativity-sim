...
class World:
    ...
    class Particle:
        ...
        def update(self, dt, meters_per_pixel, reference_frame_velocity):
            ...
            self.gamma = 1 / np.sqrt(1 - (adjusted_velocity / self.speedoflight)**2)
            adjusted_velocity = adjusted_velocity / self.speedoflight**2
            self.position = self.gamma*(self.position + (adjusted_velocity * dt) / meters_per_pixel) % self.world.screen_WIDTH

        def draw(self, screen):
            adjusted_velocity = (self.velocity - self.world.reference_frame_velocity)*self.gamma/(self.speedoflight**2)
    ...
    class Photon_emiter:
        ...
        def update(self, dt, meters_per_pixel, reference_frame_velocity, current_time):
            ...
            self.position = self.gamma * (
                        self.position + (adjusted_velocity * dt) / meters_per_pixel) % self.world.screen_WIDTH
